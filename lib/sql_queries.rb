def selects_all_female_bears_return_name_and_age
  SELECT*FROM bears.name, bears.age  WHERE gender = "F";
end

def selects_all_bears_names_and_orders_in_alphabetical_order
  SELECT*FROM bears.name FROM bears ORDER_BY bears.name ASC;
end

def selects_all_bears_names_and_ages_that_are_alive_and_order_youngest_to_oldest
  SELECT * FROM bears.name, bears.age FROM bears WHERE alive = 1, bears.age ASC; 
end
...
def counts_number_of_bears_with_goofy_temperaments
  SELECT bears.temperament COUNT(bears.temperament) FROM bears WHERE bears.temperament = 'goofy';
